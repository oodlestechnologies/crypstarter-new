<template>
	<div>
		<div class="container-fluid">
			<h3 class="staking-text">Withdraw Rewards</h3>
			<h3 id="withdraw" class="staking-text">Available Balance for Withdraw:- {{cst}}</h3>
			<div class="form-div">
			  ...
			  <input type="text" id="validatoraddress1" name="delegate_address" v-model="posts.delegate_address" placeholder="Add Validator address.." required>
			   <input type="text" id="validatoraddress2" disabled name="delegate_address" v-model="posts.delegate_address" placeholder="Add Validator address.." required>
					...
					<input type="number" id="amount" name="amount" v-model="posts.amount" placeholder="Enter Amount.." required min="1" /> -->
...
export default {
	...
	data() {
		setInterval(()=>{
			...
			let gridcell0temp = gridcell0
			let myAddress;
			if(gridcell0temp[1]){
			 myAddress = gridcell0temp[1].getAttribute('title')
			}
...
// cst1htvzmtualpdv7z6c3k6nxgwh0zuh908x6jfmt0
	methods: {
		...
		fetchData(){
			...
			axios
			.get('http://api.crypstarter.network/cosmos/distribution/v1beta1/delegators/'+myAddress+'/rewards')
			.then((ress)=>{
				// console.log(ress)
				this.validator_address=ress.data.rewards[0].validator_address
				let num=ress.data.rewards[0].reward[0].amount
				this.cst = parseFloat(num).toFixed(6);
			})
		},
		...
		postData(e) {
			...
			axios
				.post('http://faucet.crypstarter.network/api/v1/withdraw', {
